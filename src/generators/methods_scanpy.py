...
def adapt(args, args_info):
    ...
    dict(
        ...
        name='{arg[0]}', 
        description="{res}", 
        visible=dict(function="'{arg[1]}'!=eval(get_node(config.selected)['data']['{POSITION}']['{arg[0]}'])"),
    ...
    dict(
        ...
        name='{arg[0]}', 
        description="{res}", 
        visible=dict(function="str({arg[1]})!=get_node(config.selected)['data']['{POSITION}']['{arg[0]}']"),
        properties=dict(value="{arg[1]}",type="text")
    )"""
                codearg = f"""
        {arg[0]}=type_formater(kwargs["{arg[0]}"],{t}),"""
